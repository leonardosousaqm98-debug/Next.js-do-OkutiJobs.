...
type SupabaseJob = { id: string; company_id: string; title: string; description: string; requirements: string | null; country: string | null; province: string | null; city: string | null; work_mode: string | null; contract_type: string | null };
...
export default async function JobsPage() {
  ...
  let jobs: Job[] = [];
  if (supabase) {
    const { data } = await supabase.from("jobs").select("id,company_id,title,description,requirements,country,province,city,work_mode,contract_type").eq("status", "published").order("published_at", { ascending: false });

...
export default function CandidateProfilePage() {
  ...
  const completeness = useMemo(() => {
    const fields = [profile.fullName, profile.headline, profile.bio, profile.country, profile.province, profile.city, profile.currentTitle, profile.academicLevel, profile.studyField, profile.preferredWorkMode, profile.contractType, profile.availability, profile.certifications, profile.languages];
  ...
  async function saveProfile(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { ...profile, certifications: profile.certifications.split(",").map((item) => item.trim()).filter(Boolean), languages: profile.languages.split(",").map((item) => item.trim()).filter(Boolean) };

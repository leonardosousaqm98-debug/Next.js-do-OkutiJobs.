...
type CandidateProfile = {
  ...
  salaryMinAmount: string;
  salaryMaxAmount: string;
  salaryPeriod: string;
  availability: string;
...
const emptyProfile: CandidateProfile = { fullName: "", preferredLanguage: "pt", headline: "", desiredJobTitle: "", seniorityLevel: "", functionalAreas: "", bio: "", country: "Angola", province: "", municipality: "", city: "", preferredWorkMode: "", contractType: "", academicLevel: "", studyField: "", currentTitle: "", salaryMinKz: "", salaryMaxKz: "", salaryCurrency: "AOA", salaryMinAmount: "", salaryMaxAmount: "", salaryPeriod: "monthly", availability: "", willingToRelocate: false, willingToTravel: false, openToWork: true, visibility: "private", certifications: "", languages: "", experience: "", education: "", skills: "", portfolioUrl: "" };
...
export default function CandidateProfilePage() {
  ...
  const steps = ["Identidade e localização", "Preferências e remuneração", "Skills e histórico"];
  ...
  useEffect(() => {
    ...
    fetch("/api/profile").then(async (response) => {
      ...
      setProfile({ ...emptyProfile, fullName: data.profile?.fullName ?? "", preferredLanguage: data.profile?.preferredLanguage ?? "pt", headline: String(candidate.headline ?? ""), desiredJobTitle: String(candidate.desired_job_title ?? ""), seniorityLevel: String(candidate.seniority_level ?? ""), functionalAreas: listToText(candidate.functional_areas), bio: String(candidate.bio ?? ""), country: String(candidate.country ?? "Angola"), province: String(candidate.province ?? ""), municipality: String(candidate.municipality ?? ""), city: String(candidate.city ?? ""), preferredWorkMode: String(candidate.preferred_work_mode ?? ""), contractType: String(candidate.contract_type ?? ""), academicLevel: String(candidate.academic_level ?? ""), studyField: String(candidate.study_field ?? ""), currentTitle: String(candidate.current_title ?? ""), salaryMinKz: String(candidate.salary_min_kz ?? ""), salaryMaxKz: String(candidate.salary_max_kz ?? ""), salaryCurrency: String(candidate.salary_currency ?? "AOA"), salaryMinAmount: String(candidate.salary_min_amount ?? ""), salaryMaxAmount: String(candidate.salary_max_amount ?? ""), salaryPeriod: String(candidate.salary_period ?? "monthly"), availability: String(candidate.availability ?? ""), willingToRelocate: Boolean(candidate.willing_to_relocate), willingToTravel: Boolean(candidate.willing_to_travel), openToWork: candidate.open_to_work !== false, visibility: String(candidate.visibility ?? "private"), certifications: listToText(candidate.certifications), languages: listToText(candidate.languages), experience: listToText(candidate.experience), education: listToText(candidate.education), skills: listToText(candidate.skills), portfolioUrl: String(candidate.portfolio_url ?? "") });
  ...
  async function saveProfile(event: FormEvent<HTMLFormElement>) {
    ...
    if (step < steps.length) { setStep((current) => current + 1); return; }
    setSaving(true); setMessage(null); setError(null);
    const payload = { ...profile, certifications: profile.certifications.split(",").map((item) => item.trim()).filter(Boolean), languages: profile.languages.split(",").map((item) => item.trim()).filter(Boolean), functionalAreas: profile.functionalAreas.split(",").map((item) => item.trim()).filter(Boolean).slice(0, 3), experience: profile.experience.split("\n").map((item) => item.trim()).filter(Boolean), education: profile.education.split("\n").map((item) => item.trim()).filter(Boolean), skills: profile.skills.split(",").map((item) => item.trim()).filter(Boolean) };
  ...
  async function extractCv() {
    ...
    if (!response.ok || !result.extracted) setError(result.error ?? "Não foi possível analisar o CV.");
      ...
      setProfile((current) => ({ ...current, fullName: String(data.fullName || current.fullName), headline: String(data.headline || current.headline), desiredJobTitle: String(data.desiredJobTitle || current.desiredJobTitle), seniorityLevel: String(data.seniorityLevel || current.seniorityLevel), functionalAreas: Array.isArray(data.functionalAreas) ? data.functionalAreas.join(", ") : current.functionalAreas, bio: String(data.bio || current.bio), currentTitle: String(data.currentTitle || current.currentTitle), country: String(data.country || current.country), province: String(data.province || current.province), city: String(data.city || current.city), salaryCurrency: String(data.salaryCurrency || current.salaryCurrency), academicLevel: String(data.academicLevel || current.academicLevel), studyField: String(data.studyField || current.studyField), certifications: Array.isArray(data.certifications) ? data.certifications.join(", ") : current.certifications, languages: Array.isArray(data.languages) ? data.languages.join(", ") : current.languages, experience: Array.isArray(data.experience) ? data.experience.join("\n") : current.experience, education: Array.isArray(data.education) ? data.education.join("\n") : current.education, skills: Array.isArray(data.skills) ? data.skills.join(", ") : current.skills, portfolioUrl: String(data.portfolioUrl || current.portfolioUrl) }));

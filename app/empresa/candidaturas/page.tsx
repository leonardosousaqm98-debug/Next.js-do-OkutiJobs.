...
type CandidateRow = { id: string; headline: string | null; province: string | null; city: string | null; current_title: string | null; academic_level: string | null; study_field: string | null };
...
export default async function CompanyApplicationsPage() {
  ...
  const { data: jobs } = await supabase!.from("jobs").select("id,title").eq("company_id", auth.user.id).order("created_at", { ascending: false });
  ...
  const [{ data: candidates }, { data: profiles }, { data: documents }] = admin && candidateIds.length ? await Promise.all([
    admin.from("candidate_profiles").select("id,headline,province,city,current_title,academic_level,study_field").in("id", candidateIds),
  ...
  const jobMap = new Map(ownedJobs.map((job) => [job.id, job.title]));
